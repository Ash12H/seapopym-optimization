.PHONY: lint format doc publish_test publish
...
## Lint using Ruff
lint:
	poetry run ruff check ./seapopym_optimization

## Format using Ruff (ie. equivalent to Black)
format:
	poetry run ruff format ./seapopym_optimization
...
## Check if pandoc is installed, needed for markdown to rst conversion in Notebooks
	...
	poetry run sphinx-apidoc seapopym_optimization -o docs/source
