...
class OptimizationLog:
    ...
    @classmethod
    def from_pandas(cls, df: pd.DataFrame, parameter_names: list[str], objective_names: list[str]) -> OptimizationLog:
        """
    ...
    @classmethod
    def load_netcdf(cls, filepath: str) -> OptimizationLog:
...
def generate_optimization_log_with_sobol_sampling(
    functional_group_parameters: Sequence | FunctionalGroupSet,
    sample_number: int,
    fitness_names: Sequence[str],
) -> OptimizationLog:
    """
    Generate an OptimizationLog with Sobol sampling (equivalent to generate_logbook_with_sobol_sampling).
    ...
    -------
    OptimizationLog
        An OptimizationLog containing the Sobol samples.

    """
    return OptimizationLog.from_sobol_samples(
