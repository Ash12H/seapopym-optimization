"""xarray-based genetic algorithm implementation for optimization."""
...
@dataclass
class XarrayGeneticAlgorithmParameters:
    """
    Parameters for the xarray-based genetic algorithm.

    Identical to SimpleGeneticAlgorithmParameters but works with XarrayLogbook.
    """
    ...
    def __post_init__(self: XarrayGeneticAlgorithmParameters) -> None:
        """Initialize DEAP operators and normalize weights."""
    ...
    def generate_toolbox(
        self: XarrayGeneticAlgorithmParameters, parameters: Sequence[Parameter], cost_function: CostFunctionProtocol
    ) -> base.Toolbox:
...
@dataclass
class XarrayGeneticAlgorithm:
    """
    xarray-based genetic algorithm implementation.

    Uses XarrayLogbook for storing optimization results with improved data structure
    and more intuitive multidimensional analysis capabilities.

    Attributes
    ----------
    meta_parameter : XarrayGeneticAlgorithmParameters
    ...
    logbook: XarrayLogbook | None = field(default=None, repr=False)
    toolbox: base.Toolbox | None = field(default=None, init=False, repr=False)

    def __post_init__(self: XarrayGeneticAlgorithm) -> None:
        ...
        # Validate cost function weights
    ...
    def update_logbook(self: XarrayGeneticAlgorithm, logbook: XarrayLogbook) -> None:
        ...
        if self.logbook is None:
            ...
            self.logbook = XarrayLogbook(combined_dataset)

        # Save to disk if path provided
        if self.save is not None:
            self.logbook.save_netcdf(str(self.save))

    def _evaluate(self: XarrayGeneticAlgorithm, individuals: Sequence, generation: int) -> XarrayLogbook:
        """Evaluate the cost function of all individuals and create XarrayLogbook."""

        def update_fitness(individuals: list) -> list:
            """Update fitness values for invalid individuals."""
        ...
        algorithm_metadata = {
            "algorithm": "XarrayGeneticAlgorithm",
            "generation": generation,
            "population_size": len(individuals),
            "mutation_rate": self.meta_parameter.MUTPB,
            "crossover_rate": self.meta_parameter.CXPB,
            "tournament_size": self.meta_parameter.TOURNSIZE,
            "eta": self.meta_parameter.ETA,
            "indpb": self.meta_parameter.INDPB,
            "cost_function_weights": str(self.meta_parameter.cost_function_weight),
        }

        # Extract fitness values
        fitness_values = [tuple(ind.fitness.values) for ind in individuals]

        # Create XarrayLogbook with fitness values
        ...
        # Update fitness values in the logbook (they need to be set after creation)
        logbook.update_fitness(generation, list(range(len(individuals))), fitness_values)

        return logbook

    def _initialization(self: XarrayGeneticAlgorithm) -> tuple[int, list]:
        """Initialize the genetic algorithm. If a logbook is provided, it will load the last generation."""

        def create_first_generation() -> tuple[Literal[1], list]:
        ...
        def create_population_from_logbook(generation_data: xr.Dataset) -> list:
        ...
        logger.info("XarrayLogbook found. Loading last generation.")
    ...
    def optimize(self: XarrayGeneticAlgorithm) -> XarrayLogbook:
        """
        ...
        -------
        XarrayLogbook
            The optimization results in xarray format for advanced analysis.
        """
    ...
    @classmethod
    def from_simple_genetic_algorithm(
        cls, simple_ga: SimpleGeneticAlgorithm, convert_logbook: bool = True
    ) -> XarrayGeneticAlgorithm:
        """
        Create XarrayGeneticAlgorithm from SimpleGeneticAlgorithm.

        Parameters
        ----------
        simple_ga : SimpleGeneticAlgorithm
            The SimpleGeneticAlgorithm to convert.
        convert_logbook : bool
            Whether to convert the existing pandas logbook to xarray format.

        Returns
        -------
        XarrayGeneticAlgorithm
            The converted genetic algorithm.
        """
        from seapopym_optimization.algorithm.genetic_algorithm.simple_genetic_algorithm import SimpleGeneticAlgorithm

        # Convert parameters
        xarray_params = XarrayGeneticAlgorithmParameters(
            ETA=simple_ga.meta_parameter.ETA,
            INDPB=simple_ga.meta_parameter.INDPB,
            CXPB=simple_ga.meta_parameter.CXPB,
            MUTPB=simple_ga.meta_parameter.MUTPB,
            NGEN=simple_ga.meta_parameter.NGEN,
            POP_SIZE=simple_ga.meta_parameter.POP_SIZE,
            TOURNSIZE=simple_ga.meta_parameter.TOURNSIZE,
            cost_function_weight=simple_ga.meta_parameter.cost_function_weight,
        )

        # Convert logbook if requested and available
        xarray_logbook = None
        if convert_logbook and simple_ga.logbook is not None:
            parameter_names = list(
                simple_ga.cost_function.functional_groups.unique_functional_groups_parameters_ordered().keys()
            )
            fitness_names = [obs.name for obs in simple_ga.cost_function.observations]

            xarray_logbook = XarrayLogbook.from_pandas(
                df=simple_ga.logbook, parameter_names=parameter_names, objective_names=fitness_names
            )

        # Create XarrayGeneticAlgorithm
        xarray_ga = cls(
            meta_parameter=xarray_params,
            cost_function=simple_ga.cost_function,
            client=simple_ga.client,
            constraint=simple_ga.constraint,
            save=simple_ga.save,
            logbook=xarray_logbook,
        )
